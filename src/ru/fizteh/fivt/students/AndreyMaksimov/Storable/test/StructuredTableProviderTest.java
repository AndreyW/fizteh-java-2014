...
import ru.fizteh.fivt.students.MaksimovAndrey.Storable.StructuredTableProvider;
...
public class StructuredTableProviderTest {
    ...
    public void testListOfTables() throws IOException {
        ...
        assertTrue(((StructuredTableProvider) provider).getTableNames().containsAll(
                new LinkedList<>(Arrays.asList("table", "таблица", "табличка"))));
        assertEquals(((StructuredTableProvider) provider).getTableNames().size(), 3);
        provider.removeTable("table");
        assertEquals(((StructuredTableProvider) provider).getTableNames().size(), 2);
        assertTrue(((StructuredTableProvider) provider).getTableNames().containsAll(
